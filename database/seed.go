...
func Seed(db *gorm.DB) error {
	...
	for name, slug := range categories {
		...
		db.Where("slug = ?", slug).Attrs(models.Category{Name: name}).FirstOrCreate(&c)
		catMap[slug] = &c
	}
	...
	for name, slug := range tags {
		var t models.Tag
		db.Where("slug = ?", slug).Attrs(models.Tag{Name: name}).FirstOrCreate(&t)
		tagMap[slug] = &t
	}
	...
	for name, slug := range series {
		var s models.Series
		db.Where("slug = ?", slug).Attrs(models.Series{Name: name}).FirstOrCreate(&s)
		seriesMap[slug] = &s
	}
	...
	posts := []seedPost{
		{Title: "产品经理又提了个离谱需求", Slug: "pm-li-pu-xu-qiu", Category: "bathroom-break", Tags: []string{"程序员", "甲方乙方", "离谱设计"}, DaysAgo: 2, Content: pmLiPuXuQiu},
		{Title: "同事在周会上不小心共享了屏幕", Slug: "zhou-hui-she-si", Category: "lunch-break", Tags: []string{"社死现场", "打工人", "办公室"}, DaysAgo: 5, Content: zhouHuiSheSi},
		{Title: "这只猫写代码比我快", Slug: "cat-writes-code", Category: "quick-peek", Tags: []string{"动物", "程序员"}, DaysAgo: 4, Content: catWritesCode},
		{Title: "甲方说「很简单」的时候", Slug: "jia-fang-hen-jian-dan", Category: "bathroom-break", Tags: []string{"甲方乙方", "打工人"}, Series: "jia-fang-tu-jian", DaysAgo: 7, Content: jiaFangHenJianDan},
		{Title: "厕所里的神秘代码", Slug: "toilet-code", Category: "lunch-break", Tags: []string{"程序员", "互联网考古", "冷笑话"}, DaysAgo: 9, Content: toiletCode},
		{Title: "开会时如何假装在做笔记", Slug: "fake-meeting-notes", Category: "quick-peek", Tags: []string{"打工人", "办公室", "冷笑话"}, DaysAgo: 3, Content: fakeMeetingNotes},
		{Title: "老板让我做 AI 但我只会 if-else", Slug: "ai-is-if-else", Category: "quick-peek", Tags: []string{"程序员", "冷笑话"}, Series: "cheng-xu-yuan-sheng-cun-zhi-nan", DaysAgo: 6, Content: aiIsIfElse},
		{Title: "设计师和程序员之间永远解不开的结", Slug: "designer-vs-dev", Category: "bathroom-break", Tags: []string{"离谱设计", "程序员", "冷笑话"}, DaysAgo: 8, Content: designerVsDev},
		{Title: "2024 年最离谱的技术面试题", Slug: "crazy-interview-2024", Category: "daily-highlight", Tags: []string{"程序员", "打工人"}, DaysAgo: 10, Content: crazyInterview2024},
		{Title: "如何用三句话激怒一个程序员", Slug: "trigger-a-dev", Category: "quick-peek", Tags: []string{"程序员", "冷笑话"}, DaysAgo: 1, Content: triggerADev},
		{Title: "地铁上刷手机的正确姿势", Slug: "subway-phone-skills", Category: "quick-peek", Tags: []string{"打工人", "冷笑话", "办公室"}, DaysAgo: 11, Content: subwayPhoneSkills},
		{Title: "午饭时间办公室里的终极对决", Slug: "lunch-wars", Category: "lunch-break", Tags: []string{"打工人", "办公室", "奇闻"}, DaysAgo: 12, Content: lunchWars},
	}
	...
	for _, p := range posts {
		...
		var catID *int
		if cat, ok := catMap[p.Category]; ok {
			catID = &cat.ID
		}
		...
		if p.Series != "" {
			if s, ok := seriesMap[p.Series]; ok {
				var sortOrder int64
				db.Model(&models.PostSeries{}).Where("series_id = ?", s.ID).Count(&sortOrder)
...
const catWritesCode = `办公室的编程猫又火了。这只名叫 Glitch 的橘猫在主人离开时趴在键盘上，结果「写」了 47 行 Python 代码——而且能跑。
